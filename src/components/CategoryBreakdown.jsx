...
const GAP = 2 // hueco (en unidades de longitud de arco) entre segmentos

function CategoryBreakdown({ categories }) {
  const total = categories.reduce((sum, c) => sum + c.amount, 0)

  // Acumulamos el offset para encadenar los arcos del donut sin mutar
  // variables externas durante el render (reduce con acumulador).
        ...
        <ul className="breakdown__list">
          {arcs.map((arc) => (
            <li key={arc.id} className="breakdown__item">
              <span className="breakdown__icon" style={{ '--c': arc.color }}>
