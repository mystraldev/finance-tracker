...
function NetWorthHero({ accounts, history, month }) {
  const total = accounts.reduce((sum, a) => sum + a.balance, 0)

  // Variación respecto al mes anterior del historial.
  const prev = history.length > 1 ? history.at(-2).value : total
  const change = prev > 0 ? (total - prev) / prev : 0
  const positive = change >= 0

  // Cada cuenta como un segmento de la barra de composición del patrimonio.
