...
describe('TransactionForm', () => {
  it('rejects submission when the date is empty', () => {
    ...
    render(<TransactionForm accounts={accounts} categories={categories} onCancel={() => {}} onSubmit={onSubmit} />)
  ...
  it('submits when the date is valid', () => {
    ...
    expect(onSubmit).toHaveBeenCalledWith(expect.objectContaining({ date: '2026-06-10', amount: -12.5 }))
  })

  it('rejects submission with an empty amount', () => {
    render(<TransactionForm accounts={accounts} categories={categories} onCancel={() => {}} onSubmit={vi.fn()} />)
    fireEvent.click(screen.getByRole('button', { name: /Añadir movimiento/i }))
  ...
  it('submits an income transaction with positive amount', () => {
    ...
    expect(onSubmit).toHaveBeenCalledWith(expect.objectContaining({ amount: 2500, categoryId: 'income' }))
  })

  it('rejects submission without an account', () => {
    render(<TransactionForm accounts={[]} categories={categories} onCancel={() => {}} onSubmit={vi.fn()} />)
    fireEvent.change(screen.getByPlaceholderText('0,00'), { target: { value: '50' } })
    fireEvent.change(screen.getByPlaceholderText('Ej. Compra semanal'), {
      target: { value: 'Test' },
    })
